...
with st.expander("Parent Dashboard 🔒", expanded=True):
    ...
    if admin_input == ADMIN_PASSWORD:
        ...
        # ─── New: Leaderboard & Badges Reset ─────────────────────────────
        st.markdown("---")
        st.markdown("### 🏆 Leaderboard & Badges Management")
        if st.button("Reset Leaderboard & Badges"):
            for kid in data.get("kids", []):
                data["points"][kid] = 0
                data["badges"][kid] = []
            ref.set(data)
            st.success("Leaderboard points and badges have been reset!")
...
st.caption("Flynnchilada • Firebase • Streamlit")# ─── IMPORTS ─────────────────────────────────────────────────────────────
import streamlit as st
import firebase_admin
from firebase_admin import credentials, db
import json
